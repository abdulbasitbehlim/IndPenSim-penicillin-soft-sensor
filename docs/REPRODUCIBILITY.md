...
Model fitting, evaluation calculations and original result values remain unchanged. On 4 September 2026, Figure F2's plotting code in main.ipynb was updated to remove its title, label the x-axis “Batch ID” and display “Fault-inclusive HGB” in the legend. The figure was regenerated from saved results without retraining. The tests verify that reversing this recorded presentation-only edit exactly recovers the original source cell. Machine-readable verification records are indexed in [docs/README.md](README.md). Passing checks confirm repository consistency, not publication readiness or independent scientific validation.
